...
public class VTR {
	...
	static int startOptionNumber = 2;
	...
	public VTR() {
		...
		snap.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				com.virtual.view.OptionsPanel.takeSnapshot();
			}
		});
			...
			while (f.isVisible() && (grabbedFrame = grabber.grab()) != null) {
				...
				if (startOptions == true) {
					if (startOptionNumber == 1) {
						//superImposedFrame = StartOptions.getOverlaidFrame(grabbedFrame);
					} else if (startOptionNumber == 2) {
                                            try {
                                                superImposedFrame = Category.getOverlaidFrame(grabbedFrame);
                                            } catch (IOException ex) {
                                                Logger.getLogger(VTR.class.getName()).log(Level.SEVERE, null, ex);
                                            }
					}
				} else {
					if (optionsDelay >= DELAY) {
						...
						optionsDelay++;
                                            try {
                                                superImposedFrame = SnapImage.getOverlaidFrame(grabbedFrame);
                                            } catch (IOException ex) {
                                                Logger.getLogger(VTR.class.getName()).log(Level.SEVERE, null, ex);
                                            }
					}
					if (flag == true) {
                                            try {
                                                superImposedFrame = com.virtual.view.Overlay.getOverlaidFrame(
